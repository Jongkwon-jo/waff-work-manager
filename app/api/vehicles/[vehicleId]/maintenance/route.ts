...
import { createMaintenanceRecord, listMaintenanceRecords } from "@/lib/vehicle-server"
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    return NextResponse.json(
      { records: await listMaintenanceRecords(vehicleId) },
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    return NextResponse.json({ record: await createMaintenanceRecord(vehicleId, body, user) }, { status: 201 })
  } catch (error) {
    return vehicleApiErrorResponse(error)
  }
}
