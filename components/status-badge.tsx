...
const statusConfig: Record<TaskStatus, { bg: string; text: string; dot: string }> = {
  ...
  "취소": { bg: "bg-yellow-200", text: "text-yellow-800", dot: "bg-yellow-800" },
...
export function ProjectTypeBadge({ type }: { type: string }) {
  ...
  return (
    <span className={cn("inline-flex items-center rounded px-2 py-0.5 text-[11px] font-bold tracking-wide", config.bg, config.text)}>
      {type}
    </span>
  )
}
